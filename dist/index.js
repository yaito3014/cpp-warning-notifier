...
const gql = graphql2.defaults({ headers: { authorization: `token ${installationToken}` } });
...
for (const job of jobList.jobs) {
    ...
    const stepIndex = steps.findIndex((step) => step.name.match(stepRegex) &&
...
const rowHeaderFields = JSON.parse(requireEnv("INPUT_ROW_HEADERS"));
...
function generateTable(entries) {
    const columns = [...new Set(entries.map((e) => e[columnField] ?? ""))].sort((a, b) => Number(a) - Number(b));
...
async function getInstallationTokenFromWorker(workerUrl) {
    ...
    if (!tokenRequestUrl || !tokenRequestToken) {
        throw new Error("ACTIONS_ID_TOKEN_REQUEST_URL and ACTIONS_ID_TOKEN_REQUEST_TOKEN are not set. " +
    ...
    if (!tokenResponse.ok) {
        ...
        throw new Error(`Worker token exchange failed (${tokenResponse.status}): ${err.error ?? "unknown error"}`);
    }
